...
const HeaderPos = {"TOP":0, "LEFT":1, "RIGHT":2, "BOTTOM":3};
...
var HeaderManager = function() {
    ...
    this.addHeader = function(_headerPos) {
        if(!headers[_headerPos]){
            headers[_headerPos] = new Header(_headerPos);
        }
    };
...
//xHeaders.append("text").style("fill", "#1976D2").style("font-weight", 400).attr("y", function(header){ 
    ...
    ImGraph.headerManager.addHeader(HeaderPos.TOP);
    ImGraph.headerManager.addHeader(HeaderPos.BOTTOM);
    ImGraph.headerManager.addHeader(HeaderPos.LEFT);
    ImGraph.headerManager.addHeader(HeaderPos.RIGHT);
