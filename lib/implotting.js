...
compute.horizontalHeaders = function(_headers, _width, options) {
    ...
    if(options.alignment == null) {
        options.alignment = alignmentEnum.center; 
    }
    ...
    for (var i = 0; i < count; i++) {
        var textSize = _headers[i].text().trimToMaxPixels(xSteps).textSize();
        var centrum = (xSteps-textSize.width);
        
        _headers[i].x(options.padding*(i+1) + xSteps*i + textAlignmentXPos(centrum, options.alignment));
...
imPlotting = function(width, height) {
    ...
    headers = [];
    headers.push(header("123456781234567812345"));
    headers.push(header("123456781234567812345"));
    headers.push(header("123456781234567812345"));
    diagram.horizontalOptions({alignment:alignmentEnum.center});
    ...
    xHeaders.append("text").style("fill", "#1976D2").style("font-weight", 400).attr("y", function(header){ 
                ...
                return a.text().trimToMaxPixels(a.width());
            });
...
textAlignmentXPos = function(centrum, alignment) {
    value = 0;
...
String.prototype.trimToMaxPixels = function(_length, _fontWeight) {
    if (!arguments.length) _length = 30;
    if (arguments.length < 2) _fontWeight = 400;
    
    var tmp = this;
    var trimmed = this;
    while (trimmed.textSize(_fontWeight).width > _length) {
