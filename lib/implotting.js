...
Compute.verticalHeaders = function(_headers, _height, _options) {
    ...
    var height = _height - options.padding*paddingSidesCount - options.horizontalHeadersWidth;
    var ySteps = (height / (count));
    
    var oneOrMoreTruncated = false;
    var smallestLabelWidth = options.maxWidth;
    
    for (var i = 0; i < count; i++) {
        var beforeTruncateTextSize = _headers[i].textSize();
        var text = _headers[i].text().truncateWord({lenght:options.maxWidth});
        ...
        _headers[i].x(options.padding);
        _headers[i].y(options.horizontalHeadersWidth + options.padding*(i+1) + ySteps*i + beforeTruncateTextSize.height);
        _headers[i].width(options.maxWidth);
        _headers[i].height(ySteps);
    }
...
Compute.horizontalHeaders = function(_headers, _width, _options) {
    ...
    var count = _headers.length;
    var paddingSidesCount = count+1;
    var width = _width - options.padding*paddingSidesCount - options.verticalHeadersWidth;
    var xSteps = (width / (count));

    var oneOrMoreTruncated = false;
    var smallestLabelWidth = _width;
    ...
    for (var i = 0; i < count; i++) {
        ...
        _headers[i].text(text);
        _headers[i].x(options.verticalHeadersWidth + options.padding*(i+1) + xSteps*i);
        _headers[i].y(options.padding + beforeTruncateTextSize.height);
